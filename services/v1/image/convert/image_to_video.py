...
def process_image_to_video(image_url, length, frame_rate, zoom_speed, job_id, webhook_url=None):
    try:
        ...
        if zoom_speed > 0:
            ...
            video_filter = f"scale={scale_dims},zoompan=z='min(1+({zoom_speed}*{length})*on/{total_frames}, {zoom_factor})':d={total_frames}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={output_dims},fps={frame_rate}"
            ...
            # Без зума - просто статичная картинка
            video_filter = f"scale={output_dims}:force_original_aspect_ratio=decrease,pad={output_dims.replace('x', ':')}:(ow-iw)/2:(oh-ih)/2:black,fps={frame_rate}"
            logger.info(f"Video length: {length}s, Frame rate: {frame_rate}fps (static image, no zoom)")
        # =====================================
